...
class OptimizedLampreiaDensityMatrix:
    ...
    def quantum_fidelity_batch(self, rho1_batch: torch.Tensor,
        ...
        """
        Fidelidade quântica vetorizada para batch
        ...
        Returns:
            fidelities: [B]
        """
        batch_size = rho1_batch.shape[0]
        device = rho1_batch.device
        fidelities = torch.zeros(batch_size, dtype=torch.float32, device=device)

        for i in range(batch_size):
            fidelities[i] = self._fidelity_single(rho1_batch[i], rho2_batch[i])

        return fidelities

    def _fidelity_single(self, rho1: torch.Tensor, rho2: torch.Tensor) -> float:
        """Fidelidade para matrizes individuais (método simplificado)"""
        # Método simplificado para treinamento: overlap de Hilbert-Schmidt
        # F ≈ Tr(ρ1 ρ2) para matrizes de densidade normalizadas
...
class TrainableLinguisticDensityMatrix(nn.Module):
    ...
    def __init__(self,
        ...
        # U = exp(iH) onde H é Hermitiano
        self.context_hamiltonian = nn.Parameter(torch.randn(hilbert_dim, hilbert_dim))
        self.semantic_hamiltonian = nn.Parameter(torch.randn(hilbert_dim, hilbert_dim))
    ...
    def _init_parameters(self):
        ...
        # Inicializar Hamiltonianos como anti-Hermitianos
        with torch.no_grad():
            self.context_hamiltonian.data = 0.5 * (
                self.context_hamiltonian - self.context_hamiltonian.t()
            )
            self.semantic_hamiltonian.data = 0.5 * (
                self.semantic_hamiltonian - self.semantic_hamiltonian.t()
            )

    def _hamiltonian_to_unitary(self, H: torch.Tensor) -> torch.Tensor:
        """Converte Hamiltoniano anti-Hermitiano para unitário: U = exp(iH)"""
        # H é anti-Hermitiano: H† = -H
        # U = exp(iH) é unitário: U U† = I
        iH = 1j * H
    ...
    def apply_context_batch(self,
        ...
        if context_type == 'context':
            H = self.context_hamiltonian
        else:
            H = self.semantic_hamiltonian

        U = self._hamiltonian_to_unitary(H)
...
class LampreiaSemanticLoss(nn.Module):
    ...
    def _unitarity_regularization(self, model: TrainableLinguisticDensityMatrix) -> torch.Tensor:
        """Regularização para garantir unitariedade dos operadores"""
        # Para Hamiltoniano H anti-Hermitiano, U = exp(iH) deve ser unitário
        # Penalizar desvio da unitariedade: ||U U† - I||²

        # Context Hamiltonian
        U_context = model._hamiltonian_to_unitary(model.context_hamiltonian)
        I = torch.eye(model.hilbert_dim, device=U_context.device)
        context_unitarity = torch.norm(U_context @ U_context.conj().T - I)

        # Semantic Hamiltonian
        U_semantic = model._hamiltonian_to_unitary(model.semantic_hamiltonian)
        semantic_unitarity = torch.norm(U_semantic @ U_semantic.conj().T - I)

        return context_unitarity + semantic_unitarity
...
class LampreiaSemanticTrainer:
    ...
    def __init__(self,
        ...
        # Loss e otimizador
        self.criterion = LampreiaSemanticLoss(alpha=1.0, beta=0.01)
...
def demo_training():
    ...
    print("• Unitary operators via Hamiltonian exponentials")
